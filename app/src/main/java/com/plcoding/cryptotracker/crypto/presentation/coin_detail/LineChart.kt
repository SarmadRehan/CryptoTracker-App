...
fun LineChart(
    ...
    Canvas(
        ...
        val connectPoint1 = mutableListOf<DataPoint>()
        val connectPoint2 = mutableListOf<DataPoint>()
        for (i in 1 until dataPoints.size) {
            val p0 = dataPoints[i - 1]
            val p1 = dataPoints[i]
            ...
            connectPoint1.add(DataPoint(x = x, y = y1, ""))
            connectPoint2.add(DataPoint(x = x, y = y2, ""))
        }
        ...
        val linePath = Path().apply {
            if (drawPoints.isNotEmpty()) {
                ...
                for(i in 1 until drawPoints.size) {
                    cubicTo(
                        x1 = connectPoint1[i - 1].x,
                        y1 = connectPoint1[i - 1].y,
                        x2 = connectPoint2[i - 1].x,
                        y2 = connectPoint2[i - 1].y,
                        x3 = drawPoints[i].x,
                        y3 = drawPoints[i].y
                    )
                }
        ...
        drawPoints.forEachIndexed { index, dataPoint ->
            if (isShowingDataPoint) {
                val circleOffset = Offset(x = dataPoint.x, y = dataPoint.y)
